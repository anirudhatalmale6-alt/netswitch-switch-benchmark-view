...
| `rfsim/` | ggw_rfsim | RF/microwave circuit engine (AWR/ADS class): S-parameters, ABCD two-port cascade, transmission lines, microstrip syn/analysis, L-match, quarter-wave, VSWR/RL/Smith, frequency sweep, Butterworth/Chebyshev filter + bandpass LC synthesis, Touchstone .s2p import with Rollett stability K, Fourier FFT/iFFT, and an On/Off capability settings system (`settings.conf`). Plus a minimal 1D FDTD field kernel (CST/HFSS-class representative, verified vs closed-form step reflection); FEM + 3D FDTD are planned (see `rfsim/ROADMAP.md`). 38-check selftest, CPU-exact, no GPU hook. |
