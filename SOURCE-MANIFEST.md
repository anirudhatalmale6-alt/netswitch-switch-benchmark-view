...
| `switchboard/` | ggw_switchboard | Marine/industrial main electricity board monitor: SI auto-scaling across nV→TWh (~16 decades), 1φ/3φ power, Ah→Wh energy, NORMAL/WARN/ALARM range checks. 15-check selftest. |
...
| `rfsim/` | ggw_rfsim | RF/microwave circuit engine (AWR/ADS class): S-parameters, ABCD two-port cascade, transmission lines, microstrip syn/analysis, L-match, quarter-wave, VSWR/RL/Smith, frequency sweep, Butterworth/Chebyshev filter + bandpass LC synthesis, Touchstone .s2p import with Rollett stability K, Fourier DFT/FFT/iDFT with discrete-signal analysis (naive O(N²) DFT cross-checked vs radix-2 FFT, real amplitude spectrum, conjugate symmetry, Nyquist/Shannon aliasing, `signal` simulator), and an On/Off capability settings system (`settings.conf`). Plus a minimal 1D FDTD field kernel (CST/HFSS-class representative, verified vs closed-form step reflection); FEM + 3D FDTD are planned (see `rfsim/ROADMAP.md`). 46-check selftest, CPU-exact, no GPU hook. |
