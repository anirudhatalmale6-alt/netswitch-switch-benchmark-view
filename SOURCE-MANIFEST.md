...
| `rfsim/` | ggw_rfsim | RF/microwave circuit engine (AWR/ADS class): S-parameters, ABCD two-port cascade, transmission lines, microstrip syn/analysis, L-match, quarter-wave, VSWR/RL/Smith. Plus a minimal 1D FDTD field kernel (CST/HFSS-class representative, verified vs closed-form step reflection). 19-check selftest, CPU-exact, no GPU hook. |
