...
| `cputimer/` | **CPU MHz timer** (drop-in header `cpu_mhz_timer.h` + demo) — measured CPU timebase via calibrated RDTSC, loop-step ns, the √22 reduction signature, echo stays an input (no ping) | `g++` / mingw, header-only | ✓ reads 2400.0 MHz on this box, Linux + Windows .exe |
