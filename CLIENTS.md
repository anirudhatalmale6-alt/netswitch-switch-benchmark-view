...
| `loopbench/` | **loop benchmark runner** — reads DRAMM kernel Nx (real MFLOPS), CPU/OS/keyboard loops (real), GPS/radio/antenna/sensors/GPU marked NO DEVICE on a PC (real on a phone); flags 60^15000 as non-runnable, substitutes a bounded OS loop | `g++` / mingw (+ NDK for phone) | ✓ DRAMM ~485 MFLOP/s, Linux + Windows .exe |
