...
| `approve/` | **system-approval binary** — ONE single-thread single binary both server and thin client run; deterministic DRAMM workload → identical result hash everywhere; times DATA/CPU/GPU, paces to slowest; approval = SQRT((a/b+b/a)/2) timing ratio (1.0=match); APPROVED iff hash matches AND ratio within tol. Thin client self-approves if server silent | `g++` / mingw | ✓ Linux + Windows .exe, hash identical across builds (1b8ab116fff873ff), match→APPROVED, 3x skew→NOT APPROVED |
| `netswitch-sql/scaling/` | **scaling math native in MS SQL** (`ggw_scaling.sql`) — T-SQL functions for the precompute-once constant (SQRT/LOG10/POWER), hypervisor downscale, thrice-sinwave handler (SIN), lnlog (LOG(LOG)), binary→lnlog, best-scaler pick; plus tables holding registrar/voice-edge sessions (`register_session`). Math verified in `verify_math.py` (lnlog(e^e)=1) — live run on client's MS SQL box | MS SQL (T-SQL) | ✓ math verified in code; script deploy-ready (CREATE OR ALTER) |
| `ddtm/` | **DDTM measure** — takes a 64-part (8×8) SCCM block, DCT-transforms it, splits coefficients into powered (energy above threshold, the determining parts) vs unpowered (near-zero), then unquant/requants across scaler values and picks the best by dB-quality (log10) per nat-of-rate (ln); bands the choice onto throughput-per-MHz (spectral efficiency). Plugs into stream-ctl/stream-qc | `g++` / mingw | ✓ Linux + Windows .exe, DCT roundtrip 58 dB at Q=1, flat block = 1 powered/63 unpowered |
| `voice-edge/` | **voice slice — SIP register + RTP through an edge** — thin client REGISTERs with a one-time backup code (verify+burn gate), places a SIP INVITE/200/ACK call, streams real G.711 µ-law RTP (440Hz tone, 20ms packets); the media edge relays every packet back; BYE tears down. Real SIP/RTP wire format → drops onto Kamailio+rtpengine unchanged | `g++ -pthread` / mingw (`-lws2_32`) | ✓ Linux + Windows .exe (winsock/wine), selftest PASS 25/25, bad/used code → 403 |
| `registrar/` | **user + device registrar** (control plane) — a user proves identity with a one-time backup code (verify+burn), then the server logs the device's IP/GW/MASK/DNS/DNS2/DNS-peripheral/DHCP as a session; `--auto` reads real params on Linux (incl. DHCP-vs-static); README lays out the SIP/RTP voice-rerouting architecture (Kamailio + rtpengine = RTP handler/scaler + media edge, steered by the NetSwitch rerouting engine, MS SQL store) | `g++` / mingw | ✓ Linux + Windows .exe, enroll/deny/replay/list tested, real net auto-detect |
